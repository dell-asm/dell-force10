...
describe Puppet::Util::NetworkDevice::Transport_ftos::Base_ftos do
  class TestTransport < Puppet::Util::NetworkDevice::Transport_ftos::Base_ftos
  end
  ...
  describe "when sending commands" do
    it "should send the command to the telnet session" do
      @transport.expects(:send).with("line", false)
