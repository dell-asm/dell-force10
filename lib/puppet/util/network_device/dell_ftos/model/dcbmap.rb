...
class Puppet::Util::NetworkDevice::Dell_ftos::Model::Dcbmap < Puppet::Util::NetworkDevice::Dell_ftos::Model::Base
  ...
  def perform_update(is, should)
    ...
    when :present
      transport.command("dcb-map #{name}", :prompt => /\(conf-dcbmap-#{name}\)#\s?\z/n)
    ...
    when :absent
      transport.command("no dcb-map #{name}")
	  else
  ...
  def mod_path_base
    return 'puppet/util/network_device/dell_ftos/model/dcbmap'
  end

  def mod_const_base
    return Puppet::Util::NetworkDevice::Dell_ftos::Model::Dcbmap
  end
