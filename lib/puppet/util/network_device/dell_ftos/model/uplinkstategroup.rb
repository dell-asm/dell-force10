...
class Puppet::Util::NetworkDevice::Dell_ftos::Model::Uplinkstategroup < Puppet::Util::NetworkDevice::Dell_ftos::Model::Base
  ...
  def perform_update(is, should)
    ...
    when :present
      transport.command("uplink-state-group #{name}", :prompt => /\(conf-uplink-state-group-#{name}\)#\s?\z/n)
    ...
    when :absent
      transport.command("no uplink-state-group #{name}")
	  else
  ...
  def mod_path_base
    return 'puppet/util/network_device/dell_ftos/model/uplinkstategroup'
  end

  def mod_const_base
    return Puppet::Util::NetworkDevice::Dell_ftos::Model::Uplinkstategroup
  end
