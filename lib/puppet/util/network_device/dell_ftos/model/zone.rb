...
class Puppet::Util::NetworkDevice::Dell_ftos::Model::Zone < Puppet::Util::NetworkDevice::Dell_ftos::Model::Base
  ...
  def perform_update(is, should)
    ...
    when :present
      ...
      transport.command("fc zone #{name}", :prompt => /\(conf-fc-zone-#{name}\)#\s?\z/n)
    ...
    when :absent
      transport.command("no fc zone #{name}")
	else
  ...
  def mod_path_base
    return 'puppet/util/network_device/dell_ftos/model/zone'
  end

  def mod_const_base
    return Puppet::Util::NetworkDevice::Dell_ftos::Model::Zone
  end
