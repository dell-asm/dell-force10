...
# For calculation of MD5- trimming off version, last configuration changed date, startup configuration updated date sections from configuration file content and so calculating MD5 on remaining file content
...
# If configuration is startup config, then switch will be rebooted and so re-establish transport session to handle further configurations provided in site.pp
